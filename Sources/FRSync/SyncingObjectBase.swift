...
extension SyncingObjectBase {
  ...
  public static func value(documentData: [String: Any]) -> [String: Any] {
      ...
      if property.collection {
        if property.array || property.set {
          ...
          for documentDataValue in documentDataCollection {
            let propertyValue = _propertyValue(documentDataValue, for: property)
        ...
        value[property.name] = _propertyValue(documentDataValue, for: property)
      }
    }
    value[_name(for: \Self.syncedAt)] = Date()
    return value
  }
}
...
private func _propertyValue(_ documentDataValue: Any, for property: Property) -> Any {
  switch property.type {
  case .int:
  ...
  case .object:
    ...
    if objectClass.isEmbedded() {
      ...
      let documentData = documentDataValue as! [String: Any]
      return objectClass.value(documentData: documentData)
    } else {
      fatalError("Add as subcollection instead")
    }
  case .linkingObjects:
